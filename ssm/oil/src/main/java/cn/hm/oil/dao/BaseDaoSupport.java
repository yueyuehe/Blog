...
public class BaseDaoSupport extends SqlSessionDaoSupport {
	...
	public <T> List<T> getListPageSupportByManualOperation(String key, String countKey, Map<String, Object> param, PageSupport pageSupport) throws IllegalArgumentException{
		...
		if (param == null)
			param = new HashMap<String, Object>();
		param.put("limit", pageSupport.getPageSize());
		if(pageSupport.getIsInit() == false)
		{
			param.put("offset", pageSupport.getFirstPageOffset());
			pageSupport.setPageOffset(pageSupport.getFirstPageOffset());
			pageSupport.setIsInit(true);
		}
		else{
			param.put("offset", pageSupport.getPageOffset());
		}
	...
	public <T> List<T> getListPageSupportByManualOperationAutoCount(String key, Map<String, Object> param, PageSupport pageSupport) throws IllegalArgumentException{
		return this.getListPageSupportByManualOperation(key, key+"_count", param, pageSupport);
	}
}
